	...
	class RevSliderOutput{
		...
		private function putSlides(){
			...
			foreach($slides as $index => $slide){
				...
				$urlSlideImage_data = getimagesize($urlSlideImage);
				
				//Html
				?>
					<li data-transition="<?php echo $transition?>" data-slotamount="<?php echo $slotAmount?>" <?php echo $htmlParams?>>
						<img src="<?php echo $urlSlideImage?>" <?php echo $styleImage?> alt="<?php echo $alt?>" <?php echo $imageAddParams?> <?php echo $urlSlideImage_data[3];?>>
						<?php	//put video:
		...
		private function putCreativeLayer(RevSlide $slide){
			...
			?>
				...
				if($isFullWidthVideo == true):
				?>
					<div class="tp-caption fade fullscreenvideo" <?php echo $htmlVideoAutoplay.$htmlVideoNextSlide.$htmlHidden?>><?php echo $html?></div>				
