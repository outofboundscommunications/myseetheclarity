    ...
    $cat_id = get_category_to_follow(get_the_ID());
  	...
  	if(!isset($meta_values['meta_metro'][0])){
   	...
   	if($meta_values['meta_metro'][0] == 'photo'){
   		$data = $data . '<div class="post-front"><div class="flip-wrap-oct"><div class="flip-wrap-oct-inside">';
   		$data = $data . '<img src="'.$image_url[0].'" width="'.$image_url[1].'" height="'.$image_url[2].'"  alt="" />';
   		$data = $data . '</div></div></div><div class="post-back post-data-bg"><div class="flip-wrap-oct"><div class="flip-wrap-oct-inside">';
		...
		$data = $data . '<div class="post-front post-data-bg"><div class="flip-wrap-oct"><div class="flip-wrap-oct-inside">';
		$data = $data . '<div class="post-back-wrap">';
		
		$data = $data . $cats;
		
		$data = $data . '<a class="title-link" href="'. $permalink.'">'. $the_title .'</a>' . $icons_code;
		$data = $data . '</div></div></div></div><div class="post-back"><div class="flip-wrap-oct"><div class="flip-wrap-oct-inside">';
