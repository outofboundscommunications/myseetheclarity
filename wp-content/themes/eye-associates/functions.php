...
/*29-4-2016- Keyur Career Icon on Top Header */
function top_tool_careers(){
        //global $sf_data;
	//$top_tool_location = $sf_data['top_tool_location'];
...
add_image_size( 'location_home',          341, 310, true ); //
...
add_image_size( 'banner_tablet', 1024, 0, true ); //
add_image_size( 'banner_mobile', 480, 0, true ); //
/*--------------------------------------------------------------------------------
	Menu
--------------------------------------------------------------------------------*/
...
/*
===========================================================
                For Create Custom post Type
===========================================================
*/
function my_costom_post_type() {
	$args = array(
		'labels' => array(
			'name' => __( 'Seminars' ),
...
function populate_posts( $form ) {
    ...
    foreach ( $form['fields'] as &$field ) {
        if ( $field->type == 'select' && $field->cssClass == 'eventdate') {
		    ...
		    $choices = array();
			   ...
			   foreach ( $posts as $post ) {
				   ...
				   $choices[] = array( 'text' => get_field('date_and_time',$post->ID), 'value' => $post->ID );
			   }
		  // update 'Select a Post' to whatever you'd like the instructive option to be
		   $field->placeholder = '-- Select a Event Date --';
