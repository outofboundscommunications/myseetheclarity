	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<div class="entry-content">
						...
						<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 ">
							...
							<div class="location_detail">
								<div class="location-details">
									...
									<span class="location-time">
										<span class="location_label hover_menu_wrapper style-green data-fax">
											<span data-popover_data_key="holiday_hours" data-placement="bottom" data-title="" data-trigger="hover" data-toggle="popover" class="hover_menu_label" data-original-title="" title="">
												Holiday Hours
											</span>
											<span style="display:none;" class="hover_menu_data_wrapper" id="holiday_hours">
												<span class="hover_menu">
													<?php echo '<span style="color:#FFFFFF;">'.$holiday_hours.'</span>'; ?>
												</span>
											</span>
										</span>
										<span class="location_data"></span>
							...
							<div class="location-socials">
								...
								<ul>
									...
									if( $location_facebook ){
										?>
										<li class="facebook">
											<a href="<?php echo $location_facebook;?>">Facebook</a>
										</li>
										<?php
									}
								...
								<?php */ ?>
								<?php
								$location_social_profiles = get_field('location_social_profiles');
								
								// check if the flexible content field has rows of data
								if( have_rows('location_social_profiles') ){
									?>
									<ul class="loc-socials-profiles">
										<?php
										// loop through the rows of data
										while ( have_rows('location_social_profiles') ) {
											the_row();
											
											// vars
											$loc_soc_enabled = get_sub_field('loc_soc_enabled');
											$loc_soc_title   = get_sub_field('loc_soc_title');
												$social_class = strtolower(sanitize_title($loc_soc_title));
											$loc_soc_icon    = get_sub_field('loc_soc_icon');
											$loc_soc_url     = get_sub_field('loc_soc_url');
											$loc_soc_target  = get_sub_field('loc_soc_target');
												if( !$loc_soc_target ){
													$loc_soc_target = 1;
												}
											if( $loc_soc_enabled ){
												?>
												<li class="loc-socials-profile loc-socials-profile-<?php echo $social_class;?>">
													<a <?php echo ($loc_soc_target == 1)?'target="_blank"':'' ?> title="<?php echo $loc_soc_title;?>" href="<?php echo $loc_soc_url;?>" class="<?php echo $social_class; ?>">
														<?php echo $loc_soc_icon->element;?><span class="loc-socials-profile-title"><?php echo $loc_soc_title;?></span>
													</a>
												</li>
												<?php
											}
							...
							<div class="location-map">
								<?php get_template_part( 'content/location', 'map' );?>
							</div>
			...
			*/
			...
			if( $storefront ) {
				get_template_part( 'content/location', 'storefront' );
			}
			?>
